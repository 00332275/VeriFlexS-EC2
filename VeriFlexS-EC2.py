...
à flexão composta normal segundo o Eurocode 2 (2004).
...
with open('dados_ec2.xlsx', 'rb') as target:
...
def esfor(As, a1, a2, Es, epscu, epsc2, fcd, fyk, gamas, Ly, rj, X, xc, xg, yc, yg, ys):
    ...
    # domínios 1 e 2
    if X>(-1E50) and X<=((epscu*d)/(45/1000+epscu)):
        epsS = -45/1000*X/(d-X)
        epsI = 45/1000
    # domínios 3 e 4
    elif X>=((epscu*d)/(45/1000+epscu)) and X<=(d):
        epsS = -epscu
        epsI = epscu*(d-X)/X
    # domínio 4a
    elif X>=(d) and X<=(Ly):
...
out = xlsxwriter.Workbook('saída_ec2.xlsx')
